...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        def get_conversation_content(conversation_id=3):
        ...
        chat_context = await get_conversation_content(conversation_id)
        await self.send(text_data=json.dumps({"chat_context": chat_context}))
        message = f"{os.environ['workspace_rules_prompt']}. HERE IS CHAT HISTORY BETWEEN YOU AND THE USER:{chat_context}. RESPOND ACCORDING TO THE RULES. HERE IS THE USER'S REQUEST: {message}"
...
class ProjectTitleConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        #os.environ['hierarchy_context'] = workspaceTree
